...
logger = logging.getLogger("ph_scraper")
...
async def scrape_ph():
    ...
    if not api_key:
        logger.info("PH_API_KEY environment variable is not set. Skipping Product Hunt scraper.")
        return []
        ...
        async with httpx.AsyncClient(timeout=15.0) as client:
            ...
            for edge in edges:
                ...
                try:
                    ...
                    logger.error(f"Failed to save Product Hunt launch {url} to DB: {e}")
                    
            logger.info(f"Successfully scraped {count} launches from Product Hunt")
    except Exception as e:
